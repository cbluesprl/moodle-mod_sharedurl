...
$event = \mod_sharedurl\event\course_module_instance_list_viewed::create($params);
$event->add_record_snapshot('course', $course);
$event->trigger();

$strurl       = get_string('modulename', 'sharedurl');
$strurls      = get_string('modulenameplural', 'sharedurl');
$strname         = get_string('name');
$strintro        = get_string('moduleintro');
$strlastmodified = get_string('lastmodified');

$PAGE->set_url('/mod/sharedurl/index.php', array('id' => $course->id));
...
if (!$urls = get_all_instances_in_course('sharedurl', $course)) {
...
foreach ($urls as $url) {
    ...
    $icon = '';
    if (!empty($cm->icon)) {
        // each sharedurl has an icon in 2.0
    ...
    $table->data[] = array (
        ...
        format_module_intro('sharedurl', $url, $cm->id));
}
